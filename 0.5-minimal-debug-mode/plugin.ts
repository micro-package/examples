import type { Status, ValueObject, HookDefinition, Plugin, PluginAction } from "@micro-package/storyteller";
...
export type ExamplePluginValueObject = ValueObject<Status.forged, ExampleHookDefinition, ExamplePlugin>;
...
export const examplePlugin = createPlugin<typeof EXAMPLE_PLUG, ExampleActions, ExampleHookDefinition, ExamplePlugin>({
  ...
  actions: {
    exampleAddMessage:
      (valueObject: ExamplePluginValueObject) =>
      async ({ message }) => {
    ...
    exampleGetMessages: (valueObject: ExamplePluginValueObject) => async () => {
